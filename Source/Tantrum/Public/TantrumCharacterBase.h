...
private:
    ...
    // The reason we don't have a _multicastRequestPullObject() is that we move to the Interact state of the "Actions" anim FSM based on the "Is Interacting"
     ...
     * OnRep_XXX() IS ONLY CALLED ON THE SERVER, only the server replica will know this. The client cannot communicate this value to the replicas on other
     * clients directly. To tell the server to replicate the change on the replicas, you need to override GetLifetimeReplicatedProps(), which is what we're
